...
func _build_programmatic_hud() -> void:
	...
	var background := Panel.new(); background.name = "Background"; background.mouse_filter = Control.MOUSE_FILTER_IGNORE; background.add_theme_stylebox_override(&"panel", UiAtlas.warm_hud_style(2, 20, Vector4(12.0, 12.0, 12.0, 12.0))); layers.add_child(background); background.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
