...
func _bind_auxiliary_hud() -> void:
	...
	if _journal_button is SceneActionButton: (_journal_button as SceneActionButton).action_id = &""
	_journal_button.z_as_relative = true; _journal_button.z_index = 0; _journal_button.mouse_filter = Control.MOUSE_FILTER_STOP; CommerceUiStyle.transaction_action(_journal_button, &"journal"); _place_journal_button(host); _journal_button.pressed.connect(_toggle_journal)
	_journal = auxiliary.get_node("Journal") as Control
	(_journal.get_node("Background") as Panel).add_theme_stylebox_override(&"panel", UiAtlas.warm_hud_style(8, 26, Vector4(24.0, 20.0, 24.0, 22.0))); (_journal.get_node("Banner") as TextureRect).texture = UiAtlas.HUD_BUYSELL_BANNER; CommerceUiStyle.curved_title(_journal.get_node("Title") as Label, "JOURNAL", 3.0)
	var content_hud := _journal.get_node("ContentHud") as Panel; content_hud.add_theme_stylebox_override(&"panel", UiAtlas.warm_hud_style(2, 14, Vector4(18.0, 16.0, 18.0, 16.0)))
	var close_button := _journal.get_node("Close") as Button; UiAtlas.configure_close_button(close_button); close_button.pressed.connect(_close_journal)
	for tab: StringName in [&"unknown", &"fertilizers", &"decorations", &"mutagens"]:
		var button := _journal.get_node("ContentHud/Tabs/%s" % String(tab).capitalize()) as Button; CommerceUiStyle.shop_category_button(button, button.text, Color("b86a22")); button.pressed.connect(_select_journal_tab.bind(tab))

func _place_journal_button(host: Control) -> void:
	if _journal_button == null or host == null: return
	_journal_button.set_anchors_preset(Control.PRESET_TOP_LEFT); _journal_button.position = Vector2(28.0, maxf(0.0, host.size.y - 122.0)); _journal_button.size = Vector2(150.0, 37.0); _journal_button.modulate = Color.WHITE; _journal_button.self_modulate = Color.WHITE; _journal_button.show()

func _process(_delta: float) -> void:
	var blocked := _other_menu_open()
	if blocked != _last_blocked: _last_blocked = blocked; _sync()

func _sync() -> void:
	var host := get_parent() as Control; var app := get_node_or_null("/root/GameApp")
	if host == null or app == null or app.state == null: return
...
func _select_journal_tab(tab: StringName) -> void:
	_journal_tab = tab; var app := get_node_or_null("/root/GameApp"); if app != null: _refresh_journal(app)
func _toggle_journal() -> void: _journal.visible = not _journal.visible; _sync()
func _close_journal() -> void: _journal.visible = false; _sync()
...
func _configure_timer_hud() -> void:
	_timer.size = Vector2(153.0, 42.0); _timer_label.position = Vector2(50.0, 0.0); _timer_label.size = Vector2(88.0, 42.0)
	var hud := _timer.get_node("Hud") as Panel; hud.position = Vector2.ZERO; hud.size = Vector2(153.0, 42.0)
	_timer_icon = TextureRect.new(); _timer_icon.name = "FertilizerIcon"; _timer_icon.position = Vector2(5.0, -3.0); _timer_icon.size = Vector2(49.0, 47.0); Hud5Atlas.configure_icon(_timer_icon, Hud5Atlas.fertilizer_icon()); _timer.add_child(_timer_icon)
