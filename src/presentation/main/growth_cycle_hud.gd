...
const HUD_BODY_WIDTH := 153.0
const LEFT_ICON_SIZE := Vector2(49.0, 47.0)
...
func _build_hud() -> void:
	_panel = Panel.new(); _panel.name = "Hud"; _panel.position = Vector2.ZERO; _panel.size = Vector2(HUD_BODY_WIDTH, 42.0); _panel.mouse_filter = Control.MOUSE_FILTER_IGNORE; _panel.add_theme_stylebox_override(&"panel", _hud_style()); add_child(_panel)
	_stage_icon = TextureRect.new(); _stage_icon.name = "StageIcon"; _stage_icon.position = Vector2(6.0, -2.5); _stage_icon.size = LEFT_ICON_SIZE; Hud5Atlas.configure_icon(_stage_icon, Hud5Atlas.restart_icon()); _panel.add_child(_stage_icon)
	_timer_label = Label.new(); _timer_label.name = "TimerLabel"; _timer_label.position = Vector2(55.0, 0.0); _timer_label.size = Vector2(94.0, 42.0); _timer_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; _timer_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER; _timer_label.add_theme_font_size_override(&"font_size", 18); _timer_label.add_theme_color_override(&"font_outline_color", Color("3b1405")); _timer_label.add_theme_constant_override(&"outline_size", 2); _panel.add_child(_timer_label)
	_skip_button = Button.new(); _skip_button.name = "SkipButton"; _skip_button.position = Vector2(158.0, 3.0); _skip_button.size = Vector2(146.0, 36.0); add_child(_skip_button)
