...
func _ready() -> void:
	position = Vector2(position.x, 24.0); size = Vector2(HUD_WIDTH, 42.0); _build_hud(); _skip_button.pressed.connect(func() -> void: skip_requested.emit())

func set_cycle(plant: PlantState, _energy: int) -> void:
	visible = plant != null and plant.alive
	if not visible: return
	var duration := GrowthCycleService.duration(plant.growth_cycle_index); var boosted := plant.boosted_growth_cycle == plant.growth_cycle_index; var remaining := maxf(0.0, duration - plant.growth_cycle_elapsed) / (2.0 if boosted else 1.0)
	_stage_icon.texture = Hud5Atlas.stage_icon(plant.growth_cycle_index); _timer_label.text = _format(remaining); _timer_label.add_theme_color_override(&"font_color", Color("8cff91") if boosted else Color.WHITE)
	var cost := EnergyService.cycle_skip_cost(plant); _cost_label.text = str(cost); _skip_button.disabled = cost <= 0
