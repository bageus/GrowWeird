...
func _build_hud() -> void:
	...
	_skip_button = Button.new(); _skip_button.name = "SkipButton"; _skip_button.position = Vector2(136.0, 3.0); _skip_button.size = Vector2(108.0, 36.0); add_child(_skip_button)
	_cost_label = Hud5Atlas.configure_finish_button(_skip_button, Hud5Atlas.cycle_finish_left(), Hud5Atlas.cycle_finish_right())
