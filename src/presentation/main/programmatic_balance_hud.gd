...
func _build_hud() -> void:
	if get_node_or_null("Layers") != null:
		return
	var layers := Control.new()
	layers.name = "Layers"
	layers.mouse_filter = Control.MOUSE_FILTER_PASS
	add_child(layers)
	layers.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)

	var body := Panel.new()
	body.name = "BalanceArt"
	body.position = Vector2(36.0, 24.0)
	body.size = Vector2(188.0, 42.0)
	...
	value.add_theme_font_size_override(&"font_size", 22 if energy else 25)
	value.add_theme_color_override(&"font_color", Color.WHITE)
	value.add_theme_color_override(&"font_outline_color", Color(0.12, 0.04, 0.16, 1.0))
	value.add_theme_constant_override(&"outline_size", 5)
	value.mouse_filter = Control.MOUSE_FILTER_IGNORE
	layers.add_child(value)

	var plus := Button.new()
	plus.name = "AddButton" if energy else "ShopButton"
	plus.position = Vector2(188.0, 28.0)
	plus.size = Vector2(34.0, 34.0)
	plus.focus_mode = Control.FOCUS_NONE
	layers.add_child(plus)
	...
	if energy:
		...
		var timer := Label.new()
		timer.name = "Timer"
		timer.text = "Next energy in 02:00"
		timer.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		timer.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
		timer.add_theme_color_override(&"font_color", Color(0.28, 0.12, 0.04, 1.0))
		timer.add_theme_font_size_override(&"font_size", 14)
		next.add_child(timer)
...
func _apply_programmatic_style() -> void:
	...
	if body != null:
		...
		var inset := body.get_node_or_null("InnerShadow")
		if inset != null:
			inset.queue_free()
	var plus := get_node_or_null("Layers/%s" % ("AddButton" if energy else "ShopButton")) as Button
	if plus != null:
		plus.icon = null
		plus.expand_icon = false
		CommerceUiStyle.balance_plus(plus)
		plus.text = "+"
	if energy:
		var next := get_node_or_null("Layers/Next") as PanelContainer
		var timer := get_node_or_null("Layers/Next/Timer") as Label
		if next != null and timer != null:
			UiAtlas.configure_warm_timer_hud(next, timer)
