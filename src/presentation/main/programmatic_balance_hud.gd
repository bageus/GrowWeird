...
func _build_hud() -> void:
	...
	var icon := TextureRect.new(); icon.name = "BalanceIcon"; icon.position = Vector2(5.0, 5.0); icon.size = Vector2(42.0, 42.0); Hud5Atlas.configure_icon(icon, Hud5Atlas.energy_icon() if energy else Hud5Atlas.balance_coin_icon()); badge.add_child(icon)
