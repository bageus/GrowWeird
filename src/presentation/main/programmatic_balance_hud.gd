...
func _build_hud() -> void:
	...
	var icon := TextureRect.new(); icon.name = "BalanceIcon"; icon.position = Vector2(42.0, 27.0); icon.size = Vector2(34.0, 34.0); Hud5Atlas.configure_icon(icon, Hud5Atlas.energy_icon() if energy else Hud5Atlas.balance_coin_icon()); layers.add_child(icon)
