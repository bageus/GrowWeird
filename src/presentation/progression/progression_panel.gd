...
func _build_modal() -> void:
	...
	_window.position = -WINDOW_SIZE * 0.5; _window.size = WINDOW_SIZE; _overlay.add_child(_window)
	var background := Panel.new(); background.mouse_filter = Control.MOUSE_FILTER_STOP
	background.add_theme_stylebox_override(&"panel", UiAtlas.warm_hud_style(8, 26, Vector4(24.0, 20.0, 24.0, 22.0)))
	_window.add_child(background); background.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	var banner := TextureRect.new(); banner.texture = UiAtlas.HUD_BUYSELL_BANNER; banner.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	banner.stretch_mode = TextureRect.STRETCH_SCALE; banner.position = Vector2(290.0, -27.0); banner.size = Vector2(320.0, 82.0); _window.add_child(banner)
	var title := Label.new(); title.position = Vector2(320.0, -11.0); title.size = Vector2(260.0, 52.0)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; title.vertical_alignment = VERTICAL_ALIGNMENT_CENTER; _window.add_child(title)
	CommerceUiStyle.curved_title(title, "TASKS", 3.0)
	var close := Button.new(); close.position = Vector2(856.0, -18.0); close.size = Vector2(64.0, 64.0); _window.add_child(close)
...
func _build_main() -> void:
	...
	scroll.max_value = maxf(0.0, total_width - viewport.size.x); scroll.page = viewport.size.x
...
func _task_card(task: Dictionary, daily: bool) -> PanelContainer:
	...
	var title := Label.new(); title.text = String(task.title).to_upper(); title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; title.add_theme_font_size_override(&"font_size", 17); title.add_theme_color_override(&"font_color", Color("5b2b12")); box.add_child(title)
	var requirement := Label.new(); requirement.text = String(task.get("hint", task.title)); requirement.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	requirement.size_flags_vertical = Control.SIZE_EXPAND_FILL; requirement.add_theme_font_size_override(&"font_size", 14); requirement.add_theme_color_override(&"font_color", Color("693718")); box.add_child(requirement)
	...
	var reward := Label.new(); reward.text = "REWARD  +%d %s" % [int(task.reward), String(task.reward_kind).to_upper()]; reward.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	reward.add_theme_font_size_override(&"font_size", 15); reward.add_theme_color_override(&"font_color", Color("7b4513")); box.add_child(reward)
	...
	if not bool(task.unlocked):
		var lock := ColorRect.new(); lock.color = Color(0.11, 0.07, 0.045, 0.55); lock.mouse_filter = Control.MOUSE_FILTER_IGNORE; card.add_child(lock); lock.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	return card
...
func _connector(task: Dictionary) -> Control:
	...
	var fill := Panel.new(); fill.name = "Fill"; fill.position = Vector2(0.0, 139.0); fill.size = Vector2(CONNECTOR_WIDTH if bool(task.claimed) else 0.0, 5.0)
	fill.add_theme_stylebox_override(&"panel", _line_style(Color("ffc52c"))); connector.add_child(fill); return connector
...
func _card_style(active: bool, claimed: bool) -> StyleBoxFlat:
	var style := StyleBoxFlat.new(); style.bg_color = Color("d9a968") if claimed else Color("f0c982")
	style.border_color = Color("8d5b2b") if claimed else (Color("f6b817") if active else Color("a76525")); style.set_border_width_all(4 if active else 3); style.set_corner_radius_all(18)
	style.content_margin_left = 13.0; style.content_margin_top = 14.0; style.content_margin_right = 13.0; style.content_margin_bottom = 12.0
