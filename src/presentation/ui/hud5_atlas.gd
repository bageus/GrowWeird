...
static func shop_sprout_icon() -> Texture2D: return cell(4, 5)
...
static func icon_badge_style() -> StyleBoxFlat:
	var style := CommerceUiStyle.top_hud_style(28); style.shadow_size = 0; style.shadow_offset = Vector2.ZERO; style.bg_color = Color("74320f"); style.border_color = Color("ffad25"); style.set_border_width_all(4); return style
static func configure_atlas_button(button: Button, texture: Texture2D) -> void:
	...
	var normal := StyleBoxTexture.new(); normal.texture = texture
	var hover := StyleBoxTexture.new(); hover.texture = texture; hover.modulate_color = Color(1.12, 1.12, 1.12, 1.0)
	var pressed := StyleBoxTexture.new(); pressed.texture = texture; pressed.modulate_color = Color(0.92, 0.92, 0.92, 1.0)
	button.add_theme_stylebox_override(&"normal", normal); button.add_theme_stylebox_override(&"hover", hover); button.add_theme_stylebox_override(&"focus", hover); button.add_theme_stylebox_override(&"pressed", pressed); button.add_theme_stylebox_override(&"disabled", normal)
static func configure_finish_button(button: Button, left_texture: Texture2D) -> Label:
	...
	var dot := Label.new(); dot.name = "EnergyDot"; dot.mouse_filter = Control.MOUSE_FILTER_IGNORE; dot.position = Vector2(FINISH_SEGMENT_SIZE.x - 12.0, 0.0); dot.size = FINISH_SEGMENT_SIZE; dot.text = "·"; dot.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; dot.vertical_alignment = VERTICAL_ALIGNMENT_CENTER; dot.add_theme_font_override(&"font", UiAtlas.GAME_FONT); dot.add_theme_font_size_override(&"font_size", 22); dot.add_theme_color_override(&"font_color", Color("ffd229")); button.add_child(dot)
	var label := Label.new(); label.name = "Cost"; label.mouse_filter = Control.MOUSE_FILTER_IGNORE; label.position = Vector2(FINISH_SEGMENT_SIZE.x + 9.0, 0.0); label.size = Vector2(22.0, FINISH_SEGMENT_SIZE.y); label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER; label.add_theme_font_override(&"font", UiAtlas.GAME_FONT); label.add_theme_font_size_override(&"font_size", 18); label.add_theme_color_override(&"font_color", Color.WHITE); label.add_theme_color_override(&"font_outline_color", Color("5b2b12")); label.add_theme_constant_override(&"outline_size", 3); button.add_child(label)
	button.mouse_entered.connect(func() -> void: left.modulate = Color(1.12, 1.12, 1.12, 1.0); cost_segment.modulate = Color(1.12, 1.12, 1.12, 1.0)); button.mouse_exited.connect(func() -> void: left.modulate = Color.WHITE; cost_segment.modulate = Color.WHITE); return label
