...
static func configure_finish_button(button: Button, left_texture: Texture2D) -> Label:
	...
	button.mouse_entered.connect(func() -> void: left.modulate = Color(1.12, 1.12, 1.12, 1.0); cost_segment.modulate = Color(1.12, 1.12, 1.12, 1.0))
	button.mouse_exited.connect(func() -> void: left.modulate = Color.WHITE; cost_segment.modulate = Color.WHITE)
	return label
