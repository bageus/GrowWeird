...
static func prune(state: GameState, plant: PlantState, slot: StringName) -> String:
	if state == null or plant == null or plant.growth_cycle_index < 8 or state.energy < EnergyService.PRUNE_COST:
		return ""
...
static func plant_seed(state: GameState, seed_id: String, pot_id: String) -> bool:
	if state == null:
		return false
	var seed_state := InventoryService.find_seed(state.inventory, seed_id)
	var pot := state.find_pot(pot_id)
	if seed_state == null or pot == null:
		return false
	if not PropagationService.plant_seed(seed_state, pot, IdFactory.make("plant")):
		return false
