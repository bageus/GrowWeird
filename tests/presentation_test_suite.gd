...
func _test_scene_hud_contract() -> void:
	...
	_expect(hud_text.contains("WalletHud") and hud_text.contains("MoneyLabel") and hud_text.contains("ShopButton") and not hud_text.contains('name="CancelButton"'), "scene HUD: balance and shop must share a wallet block")
	...
	var fertilizer_aux := FileAccess.get_file_as_string("res://src/presentation/main/fertilizer_auxiliary_ui.tscn"); hud_text += fertilizer_aux
	_expect(hud_text.contains("OffersPanel") and hud_text.contains("RefreshOffer") and hud_text.contains("SkipOffer") and hud_text.contains("AdOffer") and hud_text.contains("FertilizerAuxiliaryUi") and hud_text.contains('name="Dim"') and hud_text.contains('name="JournalButton"') and hud_text.contains('name="Close"') and hud_text.contains('name="FinishButton"') and hud_text.contains("offset_top = 68.0") and hud_text.contains("offset_bottom = 108.0"), "scene HUD: fertilizer modal, dim, compact timer, journal, and actions must be persistent scene nodes")
	_expect(hud_text.contains("Vector2(637, 112)"), "scene HUD: fertilizer modal must fit every visible action")
	var cooldown_text := FileAccess.get_file_as_string("res://src/presentation/main/fertilizer_cooldown_overlay.tscn"); _expect(cooldown_text.contains("CooldownOverlay") and cooldown_text.contains("Next fertilizers 05:00"), "scene HUD: fertilizer cooldown overlay is missing")
	...
	_expect(hud_text.contains('name="PotCircle" type="Panel"') and hud_text.contains('name="PreviousPot" type="Button" parent="PotSelector/Layers"') and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("_configure_pot_circle") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls.tscn").contains("offset_left = 76.0") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("set_corner_radius_all(52)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains('gloss.name = "Gloss"') and FileAccess.get_file_as_string("res://src/presentation/main/pot_selector.gd").count("button_down.connect(_select_offset") == 2, "scene HUD: pot circle must stay square with arrows outside")
...
func _test_inventory_hud_contract() -> void:
	...
	var dialogs_text := FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_item_dialogs.tscn"); var grind_text := FileAccess.get_file_as_string("res://src/presentation/inventory/grind_confirmation.tscn")
	...
	_expect(inventory_script.contains("SCROLL_STEP := 120") and inventory_script.contains("_scroll_inventory"), "inventory HUD: arrow paging behavior is missing")
	...
	_expect(not dialogs_text.contains("CancelAction") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("dialogs.needs_scene_cancel()") and inventory_script.contains("context_cancel_requested") and inventory_script.contains("ContextCancelCaption") and inventory_script.contains("ContextCancelCell") and inventory_script.contains("face.offset_left = 17.0"), "inventory HUD: selected item must become the matching Cancel button")
	_expect(FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_item_dialogs.gd").contains("TransactionDialogVisual.configure") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("Vector2(466.0, 480.0)") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("Rect2(93.0, 408.0, 280.0, 54.0)") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("Rect2(36.0, 201.0, 394.0, 81.0)") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("icon.offset_left = 6.0") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("_configure_modal_dim(root)") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("count_background\"] as TextureRect).texture = null") and FileAccess.get_file_as_string("res://src/presentation/ui/transaction_dialog_visual.gd").contains("CommerceUiStyle.transaction_action") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("class CoinFace") and FileAccess.file_exists("res://assets/ui/coin.svg"), "inventory HUD: shared fixed modal transaction layout is incomplete")
	...
	_expect(grind_text.contains("RecycleBanner") and grind_text.contains("RecycleDescription") and grind_text.contains("QuantityMinus") and grind_text.contains("QuantityPlus") and grind_text.contains("RecycleButton"), "inventory HUD: shared grind confirmation layout missing")
	...
	_expect(main_scene_text.contains("CareGauge") and main_scene_text.contains("care_gauge.gd") and FileAccess.get_file_as_string("res://src/presentation/main/care_gauge.gd").contains("_bar_style") and FileAccess.get_file_as_string("res://src/presentation/main/care_gauge.gd").contains("var shine") and FileAccess.get_file_as_string("res://src/presentation/main/care_gauge.gd").contains("AUTO_HIDE_SECONDS := 10.0") and FileAccess.get_file_as_string("res://src/presentation/main/care_gauge.gd").contains('Color("ef3f36")') and FileAccess.get_file_as_string("res://src/presentation/main/tree_growth_preview.gd").contains("tree_hover_changed"), "care HUD: three-arc gauge is missing from the main scene")
	...
	_expect(inventory_script.contains("_add_stack_badge(button, count)") and inventory_script.contains("PRESET_TOP_RIGHT") and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.gd").contains("badge.position = Vector2(-30.0, 4.0)") and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.gd").contains('true, 29, "res://assets/tree/tree_05.png"'), "inventory HUD: stacked items must show their count over the asset's top-right corner")
	...
	_expect(controls_scene.contains("offset_top = 24.0\noffset_right = 224.0\noffset_bottom = 76.0") and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains("offset_top = 24.0\noffset_right = 224.0\noffset_bottom = 76.0"), "balance HUD: both visible bodies must be exactly 52 pixels high")
	_expect(controls_scene.contains("layout_id = &\"wallet\"\ndrag_handle_height = 120.0\nallow_scaling = true") and controls_scene.contains("z_index = 200") and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains("z_index = 200") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("configure_balance_hud") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("Vector2(39.0, 39.0)") and controls_scene.contains('name="BalanceArt" type="Panel"') and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains('name="BalanceArt" type="Panel"') and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains("offset_top = 17.0"), "balance HUD: coin and energy blocks must be scalable and above dialogs")
	_expect(FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.tscn").contains('[node name="WalletTopupPanel" type="Control"]\nvisible = false') and controls_scene.count("z_index = 180") == 3 and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.gd").contains("%CloseButton.modulate = Color(0.28") and FileAccess.get_file_as_string("res://src/presentation/main/main.tscn").contains("offset_bottom = 105.0") and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.tscn").contains('[node name="Confirm" type="Control" parent="."]\nunique_name_in_owner = true\nz_index = 190\nz_as_relative = false') and dialogs_text.contains('[node name="SellPopup" type="Control" parent="."]\nvisible = false\nz_index = 190\nz_as_relative = false'), "modal HUD: top-ups and transaction confirmations must stay below balances and above other UI")
	...
	_expect(FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("set_energy_topup_visible(false)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("set_wallet_topup_visible(false)") and FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.gd").contains('curved_title($Window/MenuTitle, "COINS")') and FileAccess.get_file_as_string("res://src/presentation/main/energy_topup_panel.gd").contains('curved_title($Window/MenuTitle, "ENERGY")') and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("CommerceUiStyle.topup_button") and FileAccess.get_file_as_string("res://src/presentation/ui/commerce_ui_style.gd").contains("_topup_style") and FileAccess.get_file_as_string("res://src/presentation/ui/commerce_ui_style.gd").contains("set_corner_radius_all(13)") and FileAccess.get_file_as_string("res://src/presentation/ui/commerce_ui_style.gd").contains("shadow_offset = Vector2(0.0, depth)") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("RUBLE_ICON") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("DOLLAR_ICON") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("BALANCE_PLUS_TEXTURE") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains('center.name = "PriceContent"') and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains('&"separation", 5') and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("button.offset_left = -75.0") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("button.clip_contents = false") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains('card_style.bg_color = Color("ffd078")') and FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.tscn").contains("offset_left = -96.0") and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains("offset_left = 48.0\noffset_top = 77.0\noffset_right = 218.0") and FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.gd").contains("_app().buy_coins") and FileAccess.get_file_as_string("res://src/presentation/main/energy_topup_panel.gd").contains("_app().buy_energy"), "wallet HUD: switching, curved titles, programmatic buttons, and timer placement are required")
	_expect(main_screen_script.contains("func _input(event: InputEvent)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("cancelable_menu_contains_global_point") and main_screen_script.contains("set_input_as_handled") and main_screen_script.contains("set_prune_cancel(mode == PlantView.MODE_PRUNE)") and main_screen_script.contains("_interaction_mode == PlantView.MODE_PRUNE: _on_cancel_pressed()") and main_screen_script.contains('action_id != &"prune": _cancel_action()') and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("face.offset_left = 4.0") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("ContextCancelButton") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("ContextCancelCaption") and FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_hud.gd").contains("context_cancel_requested"), "context menus: outside click and activator buttons must provide matching cancel behavior")
	...
	_expect(main_screen_script.contains("tree_growth_preview.clear_testing_preview()") and main_screen_script.contains("EnergyService.PRUNE_COST") and main_screen_script.contains("if not cutting_id.is_empty(): _cancel_action()") and main_screen_script.contains("Fertilizer added to inventory."), "fertilizer HUD: selecting an offer must report inventory acquisition instead of immediate use")
...
func _test_growth_stage_geometry() -> void:
	...
	_expect(flower_editor != null and FileAccess.file_exists("res://content/visual/tree_flower_layouts.json") and FileAccess.get_file_as_string("res://src/presentation/main/flower_layout_editor.gd").contains("RIPE_FRUIT") and FileAccess.get_file_as_string("res://src/presentation/main/flower_layout_editor.gd").contains("MOUSE_FILTER_PASS"), "flower layout: independent editor and storage are missing")
	_expect(FileAccess.get_file_as_string("res://src/presentation/main/leaf_layout_editor.gd").contains("scale_variance") and FileAccess.get_file_as_string("res://src/presentation/main/leaf_layout_editor.gd").contains("direction_variance") and FileAccess.get_file_as_string("res://src/presentation/main/leaf_layout_editor.gd").contains("HOVER_RADIUS := 96.0") and FileAccess.get_file_as_string("res://src/presentation/main/leaf_layout_editor.gd").contains("set_leaf_hover") and FileAccess.get_file_as_string("res://src/presentation/main/tree_growth_preview.gd").contains("clear_leaf_hover"), "leaf layout: point randomization and hover movement are missing")
	...
	_expect(preview.stage == 7 and preview.has_prunable_branch() and FileAccess.get_file_as_string("res://src/presentation/main/tree_growth_preview.gd").contains("_can_prune_side") and not FileAccess.get_file_as_string("res://src/presentation/main/tree_growth_preview.gd").contains("left_image.get_pixelv"), "tree preview: prune mode must leave test preview and restore domain branches")
	preview.free()
	var plant := _plant()
	...
	plant.growth_cycle_index = 10; _expect(TreeGrowthPreview.stage_for(plant) == 7, "tree preview: fruit stages must keep the intact mature asset"); plant.growth_cycle_index = 4
