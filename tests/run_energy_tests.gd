...
func _init() -> void:
	...
	state.pots[0].plant = PlantState.new(); state.pots[1].plant = PlantState.new()
	...
	_expect(int(migrated.get("energy", -1)) == 70, "save migration must preserve its historical energy grant")
