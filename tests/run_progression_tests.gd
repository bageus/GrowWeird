...
func _init() -> void:
	...
	_test_daily_tasks_reset_and_claim()
...
func _test_ordered_onboarding_and_rewards() -> void:
	...
	_expect(state.money == start_money, "tasks: completing a main task must wait for CLAIM before reward")
	_expect(TaskService.has_main_claim_block(state, registry), "tasks: completed main task should block the ribbon until claimed")
	var reward := TaskService.claim_main(state, registry, &"water_the_sprout")
	_expect(not reward.is_empty() and state.money == start_money + 15, "tasks: CLAIM should credit the first reward exactly once")
	_expect(TaskService.claim_main(state, registry, &"water_the_sprout").is_empty(), "tasks: main reward must not be claimable twice")
...
func _test_events_do_not_bank_before_unlock() -> void:
	...
	ProgressionActions.record_event(state, &"environment_changed", registry)
	_expect(not state.progression.is_completed(&"tune_environment"), "tasks: next main task must remain locked before previous CLAIM")
	TaskService.claim_main(state, registry, &"water_the_sprout")
	ProgressionActions.record_event(state, &"environment_changed", registry)
	_expect(state.progression.is_completed(&"tune_environment"), "progression: available milestone should complete after previous reward is claimed")

func _test_daily_tasks_reset_and_claim() -> void:
	var state := _state_with_pots(1)
	TaskService.ensure_daily(state, 86400 * 100)
	var login := TaskService.daily_tasks(state)[0]
	_expect(bool(login.completed), "daily tasks: login should complete when a daily set is initialized")
	var start_money := state.money
	_expect(not TaskService.claim_daily(state, &"daily_login").is_empty() and state.money == start_money + 5, "daily tasks: login claim should award five coins")
	TaskService.record_daily_event(state, &"watered")
	var watering := TaskService.daily_tasks(state)[3]
	_expect(bool(watering.completed), "daily tasks: watering event should complete its independent card")
	TaskService.ensure_daily(state, 86400 * 101)
	login = TaskService.daily_tasks(state)[0]
	watering = TaskService.daily_tasks(state)[3]
	_expect(bool(login.completed) and not bool(login.claimed), "daily tasks: a new day should reset login claim state")
	_expect(not bool(watering.completed), "daily tasks: a new day should reset other task progress")
...
func _test_fertilizer_shop_unlocks_and_nonshop_compost() -> void:
	...
	_expect(not ShopService.is_fertilizer_unlocked(state, radioactive), "shop progression: radioactive sample must start locked")
	...
	for item in catalog:
		if StringName(item.get("id", "")) == RecyclingService.COMPOST_ID: compost_listed = true
	...
	var daily := TaskService.daily_tasks(state)
	_expect(bool(daily[2].completed), "daily tasks: a successful shop purchase should complete Buy in the shop")
...
func _test_progression_save_round_trip() -> void:
	...
	state.progression.progress_by_id["__task_claim_water_the_sprout"] = 1
	state.progression.progress_by_id["__daily_progress_daily_water"] = 1
	...
	_expect(int(restored.progression.progress_by_id.get("__task_claim_water_the_sprout", 0)) == 1, "tasks save: main claim flag was lost")
	_expect(int(restored.progression.progress_by_id.get("__daily_progress_daily_water", 0)) == 1, "tasks save: daily progress was lost")
...
func _state_with_pots(count: int) -> GameState:
	var state := GameState.new()
	state.money = 200
	for index in range(count): state.pots.append(_pot("pot-%d" % (index + 1)))
	state.active_pot_id = state.pots[0].pot_id if not state.pots.is_empty() else ""
	return state

func _pot(id: String) -> PotState:
	var pot := PotState.new(); pot.pot_id = id; return pot
