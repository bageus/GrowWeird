...
func _test_care_gauge_ranges_and_stage_history() -> void:
	...
	var gauge := CareGaugeService.evaluate(pot, species); var feed := FertilizerDefinition.new(); feed.care_effects = {"growth_cycle": &"seed", "nutrition": 0.2}; pot.plant.nutrition = 0.2; FertilizerUseService.apply(pot.plant, feed, []); _expect(is_equal_approx(pot.plant.nutrition, 0.4), "fertilizer: stage boost must also feed the plant")
