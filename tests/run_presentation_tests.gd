...
func _test_inventory_hud_contract() -> void:
	...
	_expect(main_screen_script.contains("func _input(event: InputEvent)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("cancelable_menu_contains_global_point") and main_screen_script.contains("set_input_as_handled") and main_screen_script.contains("set_prune_cancel(mode == PlantView.MODE_PRUNE)") and main_screen_script.contains("_interaction_mode == PlantView.MODE_PRUNE: _on_cancel_pressed()") and main_screen_script.contains('action_id != &"prune": _cancel_action()') and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("face.offset_left = 14.0") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("ContextCancelButton") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("ContextCancelCaption") and FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_hud.gd").contains("context_cancel_requested"), "context menus: outside click and activator buttons must provide matching cancel behavior")
...
func _test_growth_stage_geometry() -> void:
	...
	_expect(preview.stage == 7 and preview.has_prunable_branch() and FileAccess.get_file_as_string("res://src/presentation/main/tree_growth_preview.gd").contains("_can_prune_side"), "tree preview: prune mode must leave test preview and restore domain branches")
	preview.free()
	var plant := _plant()
