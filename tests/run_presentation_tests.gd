...
func _test_scene_hud_contract() -> void:
	...
	_expect(not shop_scene.contains('name="Balance"') and not shop_script.contains("BalanceFrame") and shop_scene.contains("anchor_top = 0.2") and shop_script.contains("warm_hud_style(8, 26") and shop_script.contains("border_width_left = 7") and main_screen_script.count("shop_container.visible") == 1, "shop HUD: redundant balance, thin frames, unsafe top offset, or background Cancel is present")
