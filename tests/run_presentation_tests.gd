...
func _test_inventory_hud_contract() -> void:
	...
	_expect(main_screen_script.contains("func _input(event: InputEvent)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("cancelable_menu_contains_global_point") and main_screen_script.contains("set_input_as_handled"), "context menus: outside click must cancel without activating the scene below") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("ContextCancelCaption") and FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_hud.gd").contains("context_cancel_requested")
