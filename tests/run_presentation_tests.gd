...
func _test_inventory_hud_contract() -> void:
	...
	_expect(draggable_script.contains("KEY_CTRL") and draggable_script.contains("MOUSE_BUTTON_WHEEL_UP") and draggable_script.contains("scale_committed.emit") and FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.gd").contains("UiAtlas.COIN_LOTS[1000]") and FileAccess.get_file_as_string("res://src/presentation/main/energy_topup_panel.gd").contains("UiAtlas.ENERGY_LOTS[30]") and not FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("static func coin_lot"), "wallet HUD: scaling and uncropped top-up texture contract is missing")
