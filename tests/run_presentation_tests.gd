...
func _programmatic_balance_contract(message: String) -> bool:
	...
	match message:
		...
		"scene HUD: balance plus hit area must use fixed atlas coordinates":
			return balance.contains("plus.position = Vector2(188.0, 28.0)") and balance.contains("plus.size = Vector2(34.0, 34.0)") and balance.contains("Hud5Atlas.configure_atlas_button(plus, Hud5Atlas.plus_icon())")
		...
		"balance HUD: both visible bodies must be exactly 52 pixels high":
			return balance.contains("body.position = Vector2(36.0, 24.0)") and balance.contains("body.size = Vector2(188.0, 42.0)") and balance.contains("CommerceUiStyle.top_hud_style(21)") and wallet_scene.contains("offset_bottom = 100.0") and energy_scene.contains("offset_bottom = 126.0")
		"balance HUD: coin and energy blocks must be scalable and above dialogs":
			return wallet_scene.contains("z_index = 200") and energy_scene.contains("z_index = 200") and balance.contains('body.name = "BalanceArt"') and balance.contains('icon.name = "BalanceIcon"') and balance.contains("Hud5Atlas.configure_icon(icon, Hud5Atlas.energy_icon() if energy else Hud5Atlas.balance_coin_icon())") and balance.contains("style.shadow_size = 0")
