...
func _test_inventory_hud_contract() -> void:
	...
	_expect(hud_text.contains("Vector2(0, 374)") and hud_text.contains("separation = 1"), "inventory HUD: viewport must show exactly three non-overlapping 124px cells")
	...
	_expect(inventory_script.contains("offset_left = 20.6") and inventory_script.contains("offset_right = -20.6"), "inventory HUD: item art must be reduced by ten percent inside its slot")
