...
func _test_inventory_hud_contract() -> void:
	...
	_expect(FileAccess.get_file_as_string("res://src/presentation/inventory/inventory_item_dialogs.gd").contains("sell_minus.visible = can_change_quantity") and shop_script.contains("quantity_minus.visible = can_change_quantity") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("button.position = Vector2(202.0, 43.0)"), "commerce HUD: singleton quantity controls and balance button geometry are incorrect")
