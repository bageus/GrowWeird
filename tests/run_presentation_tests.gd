...
func _test_inventory_hud_contract() -> void:
	...
	_expect(hud_text.contains("Vector2(0, 404)") and hud_text.contains("separation = 1"), "inventory HUD: viewport must show exactly three non-overlapping 134px cells")
	...
	_expect(inventory_script.contains("offset_left = 21.1") and inventory_script.contains("offset_right = -21.1"), "inventory HUD: item art must remain reduced by ten percent inside its enlarged slot")
	_expect(hud_text.contains("vertical_scroll_mode = 3"), "inventory HUD: native vertical scrollbar must stay hidden")
	_expect(hud_text.contains("Vector2(178, 550)"), "inventory HUD: frame must fit three enlarged cells")
