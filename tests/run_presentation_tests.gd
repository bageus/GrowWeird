...
func _test_scene_hud_contract() -> void:
	...
	_expect(shop_scene.contains('[node name="ConfirmBackground" type="Panel" parent="Confirm"]') and shop_script.contains("warm_hud_style(8, 26") and dialogs_text.contains('[node name="SellBackground" type="Panel" parent="SellPopup"]') and dialogs_script.contains("warm_hud_style(8, 26"), "transaction HUD: purchase and sale confirmations must use framed programmatic panels")
