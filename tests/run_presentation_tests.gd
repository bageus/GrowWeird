...
func _test_growth_stage_geometry() -> void:
	var plant := _plant()
	plant.growth_ratio = 0.1
	var young := PlantVisualAssembler.build(Vector2(600.0, 400.0), plant)
	var young_slots: Dictionary = young["slots"]
	_expect(bool(young_slots["center"]["visible"]), "growth view: center should emerge first")
	_expect(not bool(young_slots["left"]["visible"]), "growth view: left branch appeared too early")
	plant.growth_ratio = 0.5
	var mature := PlantVisualAssembler.build(Vector2(600.0, 400.0), plant)
	var mature_slots: Dictionary = mature["slots"]
	_expect(bool(mature_slots["left"]["visible"]), "growth view: left branch should be visible")
	_expect(bool(mature_slots["right"]["visible"]), "growth view: right branch should be visible")
