...
func _test_inventory_hud_contract() -> void:
	...
	_expect(draggable_script.contains("KEY_CTRL") and draggable_script.contains("MOUSE_BUTTON_WHEEL_UP") and draggable_script.contains("scale_committed.emit") and FileAccess.get_file_as_string("res://src/presentation/main/wallet_topup_panel.tscn").contains("Vector2(960, 440)") and FileAccess.get_file_as_string("res://src/presentation/main/energy_topup_panel.tscn").contains("Vector2(820, 440)") and FileAccess.get_file_as_string("res://src/presentation/ui/ui_atlas.gd").contains("static func coin_lot"), "wallet HUD: scaling and compact top-up layout contract is missing")
