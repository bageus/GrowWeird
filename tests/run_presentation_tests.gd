...
func _test_inventory_hud_contract() -> void:
	...
	_expect(controls_scene.contains("offset_top = 24.0\noffset_right = 224.0\noffset_bottom = 80.0") and FileAccess.get_file_as_string("res://src/presentation/main/energy_hud.tscn").contains("offset_top = 24.0\noffset_right = 224.0\noffset_bottom = 80.0"), "balance HUD: both visible bodies must be exactly 56 pixels high")
	...
	_expect(main_screen_script.contains("func _input(event: InputEvent)") and FileAccess.get_file_as_string("res://src/presentation/main/scene_controls_overlay.gd").contains("cancelable_menu_contains_global_point") and main_screen_script.contains("set_input_as_handled"), "context menus: outside click must cancel without activating the scene below")
