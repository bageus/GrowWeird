...
func _programmatic_balance_contract(message: String) -> bool:
	...
	match message:
		...
		"scene HUD: balance plus hit area must use fixed atlas coordinates":
			return balance.contains("plus.position = Vector2(188.0, 28.0)") and balance.contains("plus.size = Vector2(34.0, 34.0)") and balance.contains("CommerceUiStyle.balance_plus(plus)")
		"balance HUD: both visible bodies must be exactly 52 pixels high":
			return balance.contains("body.size = Vector2(188.0, 42.0)") and balance.contains("value.size = Vector2(103.0, 42.0)") and balance.contains("CommerceUiStyle.top_hud_style(21)")
		"balance HUD: coin and energy blocks must be scalable and above dialogs":
			return wallet_scene.contains("z_index = 200") and energy_scene.contains("z_index = 200") and not wallet_scene.contains("allow_scaling") and not energy_scene.contains("allow_scaling") and balance.contains('body.name = "BalanceArt"') and balance.contains("style.shadow_size = 0")
