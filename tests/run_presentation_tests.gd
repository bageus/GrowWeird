...
func _test_inventory_hud_contract() -> void:
	...
	_expect(inventory_script.contains("_add_stack_badge(button, count)") and inventory_script.contains("PRESET_TOP_RIGHT") and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.gd").contains("badge.position = Vector2(-28.0, 4.0)") and FileAccess.get_file_as_string("res://src/presentation/shop/shop_panel.gd").contains('true, 29, "res://assets/tree/tree_05.png"'), "inventory HUD: stacked items must show their count over the asset's top-right corner")
